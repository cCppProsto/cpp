...
static color red_text_color     = color(colorType::Red);
...
void example_2::draw()
{
  ...
  {
    node.second.node.set_fill_color(color(colorType::Black));
    node.second.node.set_point_size(4);
    node.second.node.draw();

    number n(node.second.node.center());
    n.set_color(color(colorType::Fuchsia));
    n.setValue(node.first);
    n.set_point_size(4);
    n.draw();
  }
}
//------------------------------------------------------------------------------
void example_2::find(int aStart, int aFinish)
{
